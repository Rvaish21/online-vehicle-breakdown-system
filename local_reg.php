...
if(isset($_POST['sub']))
{
    ...
    $la=$_POST['la'];
    $lo=$_POST['lo'];
    $usrc=mysql_query("select * from local where lid='$lid'");
    ...
    $ins=mysql_query("insert into local values('','$onm','$lid','$addr','$con','$em','$nfn','local','$la','$lo','0')");
    if($ins>0)
    {
        ...
        if($ins1>0)
        {
            ...
            {
                header("location:local_reg.php?suss=1");
            }
        }
    }
}
...
if($r[3]=="local")
{
    ...
    header("location:local/staff_home.php");
}
...
{
    header("location:local_reg.php?Fail=1");
}
